...
## Geospatial Analysis Community of Practice - The University of Queensland (UGSAC)
...
As a community of practice website, the idea is that members of the community will be able to contribute posts to the UQGSAC blog. It is a good use of multiple programming skills (Git/GitHub, R, html, etc...) and the aim is to cover the main skills necessary across Git and R to empower community members to contribute material directly to the website instead of everything being funneled through a few website administrators.
...
There is a [short online book](https://bookdown.org/yihui/blogdown/yihui-xie.html) on blogdown written by the developer, Yihui and others, and a nice, more recent [article by Allison Hill](https://www.apreshill.com/blog/2020-12-new-year-new-blogdown/) on starting your own blogdown website from scratch. 

Also, see Stéphane's tutorial for UQGSAC on creating a blogdown website.

This session focuses on how to go about *contributing* to a 
...
Good place to start is making sure you have installed the `blogdown` package:

```{r eval = FALSE}
install.packages("blogdown")
```

In order for multiple people to contribute to the same website, the website is hosted on **GitHub** and **Netlify**.

As the website is already set-up, we will be dealing with Git + GitHub and R + RStudio + Rmarkdown.
...
If you need to be convinced to use Git for version control see this [article](https://peerj.com/preprints/3159v2/) and [happy git with r](https://happygitwithr.com/index.html) to git started both by Git/R guru Jenny Bryant.

See Dr Caitie Kuempel's R Ladies Brisbane [presentation](https://github.com/cdkuempel/getting_started_github) on getting started with GitHub in RStudio.

### Git Terminology

**Repository** - where a project is stored in GitHub. Think of it like a folder holding all the relevant documents that you can verson control, view history, and add collaborators. The repository or repo holds all the relevant files for the website - most of which we will not touch.
...
- A *forked* copy of the repo should now be visible in your GitHub account.
![Screenshot of a forked repo on GitHub.](images/Github-forkedrepo.png)

The `YOU/REPO' is known as **origin** for your local repo.
...
Now that we have *cloned* the repository, let's explore the file structure a little in the 'Files' tab in RStudio. It is NOT a very intuitively set-up even for intermediate users of R. For the purposes of creating a new post to add to the blog, we are mostly concerned with the `content/` directory that contains the `post/` sub-directory.
...
- An index.Rmd file has been opened and only contains a YAML header (enclose by `---`) - more on that later. Do not change the name of the .Rmd file.
...
- If you cloned the repo properly there should be a 'Git' tab in the upper right hand window in RStudio where the 'Environment' is. In the Git repo, there should be some files listed (i.e., `post/2021-11-23-New-post-name`) with different colored boxed under the 'Status' column. 
...
Some stuff will happen and if you see the line '' then it's all good and you've made your first commit! 

Knowing when and how often to commit is a bit of an art that comes with experience. In general, you want to commit changes that are related to a single problem and have a commit message that describes the why instead of the what which is recorded in the **diff**. 

There is also a `History` button on the top left corner that will list all the commits with messages you have made and you can view the diff by clicking on a commit.

See more on committing and [best practices](https://r-pkgs.org/git.html#commit-best-practices) from the R Rackages book.
...
For the purposes of contributing a post to a blogdown website, we will not worry about pulls and *fetching upstream* at all. This basically means keeping your `origin/master` repo synced with the original repo that you forked. If you stick to creating a new post bundle and **only modifying files within the post bundles** it should be okay. BUT, know that if you are using GitHub to work collaboratively staying current with the original repo is important and would use [Happy Git with R](https://happygitwithr.com/fork-and-clone.html) as your trusty guide.

So now we will *push* our commits from or local repo to our remote 'origin/master' repo on GitHub. If you see `HEAD -> main` then all good.
...
In general, you should **commit** often and then **push** now and then.
...
From [blogdown: Creating Websites with R Markdown](https://bookdown.org/yihui/blogdown/a-quick-example.html#serve-site):
...
You can also view the locally served website in a browser by clicking on the "Show in new window" button at the top left of the RStudio View pane to the right of the broom.
...
To stop serving the site locally run `blogdown::stop_server()` in the console.

### 6. Create your content 

Once the website is set-up, forked, and cloned... you can get away with creating a new post with minimal coding. The main thing you will need to use is:
...
When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. For a website post, knitting is not as important because we can serve our site locally (which automatically knits anything new) and view the changes.

Now let's add some information about R Markdown:

##### YAML header

The YAML, or Yet Another Markdown Language, header at the top of the .Rmd is set between `---` tags. Here is where information like the Title, Date, Author of the document go.
...
To enter equations like a code chunk, use two `$` on separate lines surrounding your equations.
...
Chunks are the gray areas in the .Rmd file where you can add code that will be run. These are defined by three backticks (not a single quote, the key to the left of the 1). You can insert R code chunks but also in other languages! See the green 'Insert' code chunk button to see different options. An R code chunk will have the an `{r  }` after the opening backticks. 
...
It is also a good idea to name your chunks as chunks are included in the document outline. Chunks **cannot** have the same error - you will get an error.
...
To insert an image or gif from a website you add a `!` before the square brackets like so: `![description](https://media.giphy.com/media/u2pmTWUi0MXjyrMaVj/giphy.gif)`

test: ![happy git](https://happygitwithr.com/img/fork-triangle-happy.png)

Note: The link must end in the appropriate file extension to work.
...
![ResBaz QLD 2021 event poster](images/ResBazQld2021.jpg)

You can also save files (like images, html presentations) in your post bundle to link using *relative paths* on your own. 

### Pull Request

Okay, so as you were creating the content of your post you should have been **committing** regularly and then **pushing** less regularly, right?
...
Because we are not owner/developers of the upstream `geocommunity/blog_website` repo we need to submit a **pull request** to submit our new blog post for acceptance into the upstream repo.
...
- Once you are happy, click the green `` button at the end and wait for approval. You can have a bit of a conversation to hash out any issues as well.
